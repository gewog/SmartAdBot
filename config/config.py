...
"""
Telegram бот на основе aiogram с поддержкой FSM (finite state machine).
Этот модуль инициализирует бота, диспетчер и загружает конфигурацию из .env файла или переменных окружения.
Основные компоненты:
- Bot: Экземпляр бота для взаимодействия с Telegram API.
- Dispatcher: Диспетчер для обработки сообщений и событий.
- MemoryStorage: Хранилище для состояний FSM в оперативной памяти.
...
API_TOKEN: str = config.get("TOKEN") or os.environ.get("TOKEN")
ADMIN = int(config.get("ADMIN_ID") or os.environ.get("ADMIN_ID"))
...
if not all([API_TOKEN, ADMIN, SQLALCHEMY_URL]):
    raise ValueError("Отсутствуют обязательные переменные окружения: TOKEN, ADMIN_ID или SQLALCHEMY_URL")
...
# print("Loaded config:", {"TOKEN": "***", "ADMIN_ID": ADMIN, "SQLALCHEMY_URL": SQLALCHEMY_URL})
